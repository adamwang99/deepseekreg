#!/usr/bin/env python3
"""
DeepSeek Account Creator — Desktop App
Chạy: pip install pywebview && python3 run.py
"""

import sys, os, json, re, time, random, string, html as hm, http.cookiejar, urllib.request, urllib.parse
import sqlite3, csv, io, threading, queue
...
try:
    ...
    print("❌ Cần cài: pip install pywebview")
    print("Sau đó chạy: python3 run.py")
    sys.exit(1)
...
HOST = '127.0.0.1'
PORT = 8080
SIGNATURE = "Made by Long SRO by order of Adam Wang CEO"
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS accounts
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  email TEXT UNIQUE, password TEXT, domain TEXT,
                  proxy TEXT, status TEXT DEFAULT 'created',
...
def export_csv():
    rows = get_accounts(); out = io.StringIO(); w = csv.writer(out)
...
def export_txt():
    rows = get_accounts()
    lines = ["# DeepSeek Accounts", "# Format: email|password|domain", f"# {SIGNATURE}", "---"]
    for r in rows: lines.append(f"{r[0]}|{r[1]}|{r[2]}")
    return '\n'.join(lines)

# === Workers ===
worker_running = False; worker_logs = []; logs_lock = threading.Lock()
proxy_config = None
...
def add_log(msg):
    with logs_lock:
        ...
        if len(worker_logs) > 200: worker_logs.pop(0)

class ProxyConfig:
    def __init__(self, auth_str=''):
        self.proxies = []; self.current = 0; self.lock = threading.Lock()
        self.parse(auth_str)
    def parse(self, s):
...
def create_email(domain):
    ...
    try:
        ...
        for m in re.finditer(r'wire:id="([^"]+)"[^>]*wire:initial-data="([^"]+)"', content):
            ...
            if d.get('fingerprint',{}).get('name') == 'frontend.actions':
                ...
                payload = {'fingerprint': d['fingerprint'], 'serverMemo': d['serverMemo'],
                    'updates': [
    ...
    except Exception as e: add_log(f'  ❌ Email error: {e}')
...
def register_one(domain, proxy, password):
    add_log(f'📧 Creating email ({domain})...')
    email, cj, lw_token = create_email(domain)
    if not email: add_log(f'  ❌ Email failed'); return None
    add_log(f'  ✅ {email}')
    ...
    try:
        with sync_playwright() as p:
            ...
            ctx = browser.new_context(
                ...
                viewport={'width': 390, 'height': 844}, locale='zh-CN', proxy=proxy)
            ctx.add_init_script('Object.defineProperty(navigator, "webdriver", { get: () => undefined });')
            page = ctx.new_page()
            page.goto('https://chat.deepseek.com/', wait_until='commit', timeout=30000)
            time.sleep(8)
            page.locator('text=立即注册').first.click(); time.sleep(2)
            inp = page.locator('input')
            inp.nth(0).fill(email); inp.nth(1).fill(password); inp.nth(2).fill(password)
            page.locator('text=发送验证码').first.click(); time.sleep(5)
            text = page.evaluate('document.body?.innerText')
            if '验证码已发送' not in text and '秒后可再次获取' not in text:
                add_log(f'  ❌ Code send failed'); ctx.close(); return None
            add_log(f'  ✅ Code sent, polling OTP...')
            otp = poll_otp(cj, lw_token)
            if not otp: add_log(f'  ❌ No OTP'); ctx.close(); return None
            add_log(f'  🔑 OTP: {otp}')
            inp2 = page.locator('input'); inp2.nth(3).fill(otp)
            page.locator('button:has-text("注册")').first.click(); time.sleep(5)
            if 'chat' in page.url and 'sign' not in page.url.lower():
                add_log(f'  ✅✅ SUCCESS!'); ctx.close()
                return (email, password)
            else: add_log(f'  ⚠️ Register issue'); ctx.close(); return None
...
def worker_main(proxy_conf, password, target):
    global worker_running
    domain_idx = 0; completed = 0; fails = 0
    while worker_running and completed < target:
        domain = DOMAINS[domain_idx % len(DOMAINS)]; domain_idx += 1
        add_log(f'\n[{completed+1}/{target}] {domain}')
        proxy = proxy_conf.next()
        result = register_one(domain, proxy, password)
        if result:
            email, pw = result
            save_account(email, pw, domain, proxy['server'] if proxy else 'direct')
            completed += 1; fails = 0
        else:
            fails += 1
            if fails >= 3: add_log(f'  ⚠️ 3 fails, stopping...'); break
    worker_running = False
    add_log(f'\n{"="*40}\n✅ Done: {completed} accounts\n{"="*40}')

# === HTTP Server ===
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        path = self.path.split('?')[0]
        if path == '/': self.send_html(INDEX_HTML)
        elif path == '/api/accounts': self.send_json(get_accounts())
        elif path == '/api/logs':
            with logs_lock: self.send_json(list(worker_logs))
        elif path == '/api/status':
            self.send_json({'running': worker_running, 'count': len(get_accounts())})
        elif path == '/api/config':
            self.send_json({'proxy': get_config('proxy',''), 'password': get_config('password','abcABC123@@'),
                          'target': get_config('target','10'), 'workers': get_config('workers','1')})
        elif path == '/export/csv':
            self.send_response(200)
            self.send_header('Content-Type', 'text/csv')
            self.send_header('Content-Disposition', 'attachment; filename="deepseek_accounts.csv"')
            self.end_headers(); self.wfile.write(export_csv().encode())
        elif path == '/export/txt':
            self.send_response(200)
            self.send_header('Content-Type', 'text/plain')
            self.send_header('Content-Disposition', 'attachment; filename="deepseek_accounts.txt"')
            self.end_headers(); self.wfile.write(export_txt().encode())
        else: self.send_error(404)
    def do_POST(self):
        length = int(self.headers.get('Content-Length',0))
        body = self.rfile.read(length).decode() if length else '{}'
        data = json.loads(body) if body else {}
        if self.path == '/api/start': self.handle_start(data)
        elif self.path == '/api/stop':
            global worker_running; worker_running = False; self.send_json({'ok':True,'msg':'Stopping...'})
        elif self.path == '/api/clear':
            conn = sqlite3.connect(DB_FILE); conn.execute("DELETE FROM accounts"); conn.commit(); conn.close()
            with logs_lock: worker_logs.clear()
            self.send_json({'ok':True})
        elif self.path == '/api/save-config':
            for k in ['proxy','password','target','workers']:
                if k in data: save_config(k, str(data[k]))
            self.send_json({'ok':True})
        else: self.send_json({'error':'unknown'})
    def handle_start(self, data):
        global worker_running, proxy_config
        if worker_running: self.send_json({'ok':False,'msg':'Already running'}); return
        ...
        save_config('proxy',proxy_str); save_config('password',password); save_config('target',str(target))
        proxy_config = ProxyConfig(proxy_str); worker_running = True
        with logs_lock: worker_logs.clear()
        add_log(f'🚀 Starting: {target} accounts')
        add_log(f'📋 Proxy: {len(proxy_config.proxies)} configured')
        t = threading.Thread(target=worker_main, args=(proxy_config, password, target), daemon=True)
        t.start()
        self.send_json({'ok':True,'msg':f'Started, target {target}'})
...
.footer{{text-align:center;color:#aaa;font-size:12px;margin-top:20px;padding:15px;border-top:1px solid #eee}}
</style>
</head>
<body>
<div class="container">
<h1>🚀 DeepSeek Account Creator</h1>
<div class="subtitle">Made by Long SRO by order of Adam Wang CEO</div>
...
<div class="footer">Made by Long SRO by order of Adam Wang CEO</div>
</div>

<script>
let pollTimer=null;
function loadConfig(){{fetch('/api/config').then(r=>r.json()).then(d=>{{document.getElementById('proxyInput').value=d.proxy||'';document.getElementById('passwordInput').value=d.password||'abcABC123@@';document.getElementById('targetInput').value=d.target||'10';}})}}
function start(){{const data={{proxy:document.getElementById('proxyInput').value,password:document.getElementById('passwordInput').value,target:parseInt(document.getElementById('targetInput').value)||10}};fetch('/api/save-config',{{method:'POST',body:JSON.stringify(data)}});fetch('/api/start',{{method:'POST',body:JSON.stringify(data)}}).then(r=>r.json()).then(d=>{{if(d.ok){{pollTimer=setInterval(poll,1000);updateButtons(true)}}else alert(d.msg)}})}}
function stop(){{fetch('/api/stop',{{method:'POST'}}).then(()=>{{updateButtons(false)}})}}
function clearAll(){{if(!confirm('Clear all accounts?'))return;fetch('/api/clear',{{method:'POST'}}).then(()=>{{loadAccounts();loadLogs()}})}}
function poll(){{fetch('/api/status').then(r=>r.json()).then(d=>{{document.getElementById('totalAccounts').textContent=d.count;if(!d.running){{clearInterval(pollTimer);pollTimer=null;updateButtons(false)}}}});loadLogs();loadAccounts()}}
function loadLogs(){{fetch('/api/logs').then(r=>r.json()).then(logs=>{{const box=document.getElementById('logBox');box.innerHTML=logs.join('\\n');box.scrollTop=box.scrollHeight}})}}
function loadAccounts(){{fetch('/api/accounts').then(r=>r.json()).then(rows=>{{document.getElementById('accountsBody').innerHTML=rows.map(r=>'<tr><td>'+r[0]+'</td><td>'+r[1]+'</td><td>'+r[2]+'</td><td style=\"font-size:11px\">'+(r[3]||'direct')+'</td><td>'+(r[4]||'created')+'</td><td>'+(r[5]||'')+'</td></tr>').join('');document.getElementById('totalAccounts').textContent=rows.length}})}}
function exportCSV(){{window.location='/export/csv'}}
function exportTXT(){{window.location='/export/txt'}}
function updateButtons(r){{document.getElementById('btnStart').disabled=r;document.getElementById('btnStop').disabled=!r;document.getElementById('workerStatus').textContent=r?'▶ Running':'⏹ Stopped';document.getElementById('statusIndicator').className=r?'stat running':'stat stopped'}}
loadConfig();loadAccounts();setInterval(loadLogs,3000);setInterval(loadAccounts,5000);
</script>
</body>
</html>"""
